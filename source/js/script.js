...
const headerMainLoginButton = headerMain.querySelector(`.header-main__login-button`);
...
const featureButtons = document.querySelectorAll(`.section-features__item:not(.section-features__item--action) .section-features__item-link`);
...
const addFeatureModalLetter = () => {
    const featureModal = document.querySelector(`.modal--opened`);
    const featureModalWrapper = featureModal.querySelector(`.modal-feature__wrapper`);

    const featureModalLetterElement = document.createElement(`div`);
    featureModalLetterElement.classList.add(`modal-feature__id`);
    featureModalLetterElement.innerHTML = String.fromCharCode(64 + Number.parseInt(featureModal.dataset.feature));
...
if (slides.length > 0) {
    prevButton.addEventListener(`click`, function () {
        const activeSlide = document.querySelector(`.section-cases__wrapper--shown`);
        const prevSlide = document.querySelector(`[data-case="${Number.parseInt(activeSlide.dataset.case) - 1}"]`);
    ...
    nextButton.addEventListener(`click`, function () {
        const activeSlide = document.querySelector(`.section-cases__wrapper--shown`);
        const nextSlide = document.querySelector(`[data-case="${Number.parseInt(activeSlide.dataset.case) + 1}"]`);
...
const faqButtons = document.querySelectorAll(`.section-faq__description-item-button`);
...
if (faqButtons.length > 0) {
    faqButtons.forEach( faqButton => {
        faqButton.addEventListener(`click`, function () {
            faqButton.parentElement.classList.toggle(`section-faq__description-item--active`);
        });
    } );
}
...
const tariffsButtons = document.querySelectorAll(`.section-tariffs__item-button`);
...
if (tariffsButtons.length > 0) {
    tariffsButtons.forEach( tariffsButton => {
        tariffsButton.addEventListener(`click`, function () {
            tariffsButton.parentElement.classList.toggle(`section-tariffs__item--active`);
        });
    } );
}
